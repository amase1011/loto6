...
function parseCsv(text) {
  ...
  for (const line of lines.slice(1)) {
    ...
    rows.push({
      ...
      bonus,
      firstPrizeWinners: parseAmount(columns[9]),
      firstPrizeAmount: parseAmount(columns[14]),
      carryover: parseAmount(columns[19])
    });
  }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (requestUrl.pathname === '/api/health') {
      return send(res, 200, JSON.stringify({ ok: true, version: '2.3.0' }));
    }
...
server.listen(PORT, HOST, () => {
  console.log(`Loto6 Analyzer v2.3 running on ${HOST}:${PORT}`);
});
